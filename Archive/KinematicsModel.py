...
class kinematics():
    ...
    def leg_IK(self, xyz, rot = [0,0,0], legID=0, is_radians=True, center_offset=[0,0,0]):
        ...
        is_right = (legID in self.right_legs) #Is the leg ID one of the right ones?
...
def update_plot(pitch, roll, yaw):
    kin = Kinematics()
    xyz = np.zeros((4, 3))  # Assuming the legs are at the default position
    rot = [radians(pitch), radians(roll), radians(yaw)]
    kin.plot_robot(xyz, rot=rot)

pitch_slider = widgets.FloatSlider(value=0.0, min=-45.0, max=45.0, step=1.0, description='Pitch')
roll_slider = widgets.FloatSlider(value=0.0, min=-45.0, max=45.0, step=1.0, description='Roll')
yaw_slider = widgets.FloatSlider(value=0.0, min=-45.0, max=45.0, step=1.0, description='Yaw')

interact(update_plot, pitch=pitch_slider, roll=roll_slider, yaw=yaw_slider)
